...
import { DEFAULT_PORTFOLIO, type PortfolioConfig, type MusicTrack, resolvePortfolioMediaUrl } from '../lib/portfolio';
...
/* ======================================================================
   ...
   ====================================================================== */
function Slide1_FeatureFilm({ data }: { data: PortfolioConfig['slide1'] }) {
  ...
  return (
    <div className="w-screen h-[100dvh] shrink-0 flex flex-col md:flex-row bg-[#080808]">
      ...
      <div className="relative w-full md:w-[45%] lg:w-[40%] h-1/2 md:h-full flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/5 overflow-hidden shrink-0 bg-[#080808]">
        <TextContainerAmbient watermark={data.num || '01'} glowColor="bg-[#b20710]" />
        <div className="relative z-10 w-full h-full flex flex-col justify-center px-6 py-6 md:p-16 lg:p-24 overflow-y-auto custom-scrollbar">
          <SectionTitle num={data.num || '01'} text={data.title || 'Theatrical Release'} />
          <SectionSubhead text={data.subtitle || 'DUSSEHRA'} />
      ...
      <div className="w-full md:w-[55%] lg:w-[60%] h-1/2 md:h-full relative overflow-hidden bg-[#111] flex items-center justify-center p-4 md:p-12 lg:p-24">
        ...
        <div className="w-full max-w-4xl aspect-video relative z-10 rounded-xl md:rounded-[1rem] lg:rounded-[2rem] overflow-hidden bg-black shadow-[0_30px_60px_rgba(0,0,0,0.8)] border border-white/10 group/player">
          {isPlaying ? (
            <iframe 
              src={`https://www.youtube.com/embed/${data.youtubeId}?autoplay=1&mute=0&controls=1&rel=0`}
              title={data.title}
...
/* ======================================================================
   SLIDE 2: VFX & ANIME 
   ====================================================================== */
function Slide2_VFXAnime({ data }: { data: PortfolioConfig['slide2'] }) {
  ...
  return (
    <div className="w-screen h-[100dvh] shrink-0 flex flex-col md:flex-row bg-[#080808]">
      {/* Left Intro */}
      <div className="relative w-full md:w-[45%] lg:w-[40%] h-1/2 md:h-full flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/5 overflow-hidden shrink-0 bg-[#080808]">
        <TextContainerAmbient watermark={data.num || '02'} glowColor="bg-white" />
        <div className="relative z-10 w-full h-full flex flex-col justify-center px-6 py-4 md:p-16 lg:p-24 overflow-y-auto custom-scrollbar">
          <SectionTitle num={data.num || '02'} text={data.title || 'Visual Effects'} />
          ...
          <div className="min-h-[140px] md:min-h-[220px] shrink-0 flex flex-col">
            <AnimatePresence mode="wait">
              ...
              >
                <SectionSubhead text={activeTab.subtitle} />
                ...
                {activeTab.youtubeId ? (
                  <div className="shrink-0 flex flex-wrap items-center gap-8">
                    ...
                    <button className="flex items-center gap-2 uppercase font-mono text-xs tracking-widest font-bold text-white hover:text-[#D4AF37] transition-colors bg-transparent border-none outline-none">
                      <Play className="w-5 h-5 fill-current" />
                      VFX Showcase
                    </button>
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab.youtubeId ? (
              isPlaying ? (
                <iframe 
                  src={`https://www.youtube.com/embed/${activeTab.youtubeId}?autoplay=1&mute=0&controls=1&rel=0`}
                  className="w-full h-full border-0 absolute inset-0 text-white"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                />
...
function Slide3_Music({ data }: { data: PortfolioConfig['slide3'] }) {
  const [activeIdx, setActiveIdx] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [isMuted, setIsMuted] = useState(false);
  const [volume, setVolume] = useState(80);
  const [player, setPlayer] = useState<any>(null);
  const track = data.tracks[activeIdx];
  const thumb = track ? `https://img.youtube.com/vi/${track.youtubeId}/hqdefault.jpg` : '';
  ...
  useEffect(() => {
    ...
    (window as any).onYouTubeIframeAPIReady = () => {
      if (data.tracks[activeIdx]) {
        createPlayer(data.tracks[activeIdx].youtubeId, 0);
      }
    };

    if ((window as any).YT && (window as any).YT.Player && data.tracks[activeIdx]) {
      createPlayer(data.tracks[activeIdx].youtubeId, 0);
    }
  ...
  const createPlayer = (videoId: string, startMs: number) => {
    if ((window as any).YT && (window as any).YT.Player) {
      ...
      const newPlayer = new (window as any).YT.Player('yt-player-instance', {
        ...
        width: '300',
        videoId: videoId,
        playerVars: { autoplay: 0, controls: 0, disablekb: 1, fs: 0, rel: 0, start: startMs, modestbranding: 1 },
        events: {
          onReady: (event: any) => {
  ...
  useEffect(() => {
    if (player && player.loadVideoById && track) {
  ...
  return (
    <div ref={containerRef} className="w-screen h-[100dvh] shrink-0 flex flex-col md:flex-row bg-[#080808]">
      ...
      <div className="relative w-full md:w-[45%] lg:w-[40%] h-1/2 md:h-full flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/5 shrink-0 overflow-hidden bg-[#0a0a0a]">
        <TextContainerAmbient watermark={data.num || '03'} glowColor="bg-[#FF0000]/20" />
        <div className="relative z-10 w-full h-full flex flex-col justify-center px-6 py-6 md:p-16 lg:p-24 overflow-y-auto custom-scrollbar">
          <SectionTitle num={data.num || '03'} text={data.title || 'Music Production'} />
      ...
      <div className="w-full md:w-[55%] lg:w-[60%] h-1/2 md:h-full flex flex-col p-6 md:p-12 lg:p-24 z-10 bg-[#000000]/60 backdrop-blur-md overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-white/40 text-[9px] md:text-xs font-bold tracking-[0.2em] uppercase">Queue • {data.tracks.length} Tracks</div>
          </div>
          ...
          {data.tracks.map((t, i) => (
            <div 
              ...
              onClick={() => { 
                if (activeIdx !== i) {
                  setIsPlaying(false);
                  setTimeout(() => { setActiveIdx(i); setIsPlaying(true); }, 50);
                } else {
            ...
            >
              ...
              <div className="text-white/30 text-[10px] md:text-xs font-mono mr-2 hidden md:block">{t.time}</div>
...
/* ======================================================================
   ...
   ====================================================================== */
function Slide4_AI({ data }: { data: PortfolioConfig['slide4'] }) {
  const [isPlaying, setIsPlaying] = useState(false);
  ...
  return (
    <div className="w-screen h-[100dvh] shrink-0 flex flex-col md:flex-row bg-[#080808]">
      ...
      <div className="relative w-full md:w-[45%] lg:w-[40%] h-1/2 md:h-full flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/5 overflow-hidden shrink-0 bg-[#0a0a0a]">
        <TextContainerAmbient watermark={data.num || '04'} glowColor="bg-white" />
        <div className="relative z-10 w-full h-full flex flex-col justify-center px-6 py-6 md:p-16 lg:p-24 overflow-y-auto custom-scrollbar">
          <SectionTitle num={data.num || '04'} text={data.title || 'AI Animation'} />
          <SectionSubhead text={data.subtitle || 'Advanced Tech'} />
          <SectionDesc>{data.description}</SectionDesc>
      ...
      <div className="w-full md:w-[55%] lg:w-[60%] h-1/2 md:h-full relative overflow-hidden bg-[#000] flex items-center justify-center p-4 md:p-12">
        ...
        >
          {isPlaying ? (
            <iframe 
              src={`https://www.youtube.com/embed/${data.youtubeId}?autoplay=1&mute=0&controls=1&rel=0&showinfo=0`}
...
export default function Portfolio() {
  const { value: config } = useSetting<PortfolioConfig>('portfolio', DEFAULT_PORTFOLIO);
  ...
  return (
    <section id="portfolio" ref={targetRef} className="relative md:h-[400vh] h-[100dvh] bg-black font-sans selection:bg-[#D4AF37]/30">
      ...
      >
        ...
        >
          <div className="w-screen h-full shrink-0 snap-center md:snap-none"><Slide1_FeatureFilm data={config.slide1} /></div>
          <div className="w-screen h-full shrink-0 snap-center md:snap-none"><Slide2_VFXAnime data={config.slide2} /></div>
          <div className="w-screen h-full shrink-0 snap-center md:snap-none"><Slide3_Music data={config.slide3} /></div>
          <div className="w-screen h-full shrink-0 snap-center md:snap-none"><Slide4_AI data={config.slide4} /></div>
        </motion.div>
