...
import { DEFAULT_TEAM, resolveTeamImageUrl } from '../lib/team';

export default function Team() {
  const containerRef = useRef<HTMLDivElement>(null);
  const { value: team } = useSetting('team', DEFAULT_TEAM);
  ...
  return (
    <div id="team" ref={containerRef} className="md:min-h-screen py-16 md:py-32 px-6 md:px-12 flex flex-col justify-center relative z-20 overflow-hidden bg-transparent">
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[url('/bts4.webp')] bg-cover bg-center opacity-10 mix-blend-screen grayscale"></div>
      ...
      <div className="max-w-5xl mx-auto w-full perspective-[1200px]">
        ...
        >
          <div className="text-center mb-16">
            <h3 className="text-[#D4AF37] uppercase tracking-[0.3em] text-xs md:text-sm mb-6 font-sans font-bold">{team.eyebrow}</h3>
            <h2 className="font-serif text-4xl md:text-6xl lg:text-7xl text-white">{team.title}</h2>
          </div>
            ...
            {team.members.map((member, i) => (
              ...
              >
                <div className="aspect-[3/4] overflow-hidden bg-gray-200 mb-6 relative">
                  <img src={resolveTeamImageUrl(member.imagePath)} alt={member.name} className="w-full h-full object-cover grayscale-0 group-hover:grayscale-[60%] transition-all duration-700 scale-100 group-hover:scale-105" />
